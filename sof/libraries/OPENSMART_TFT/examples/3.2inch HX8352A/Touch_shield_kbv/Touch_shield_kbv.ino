...
void setup(void)
{
    ...
    switch (Orientation) {      // adjust for different aspects
        case 2:   break;        //no change,  calibrated for PORTRAIT
        case 1:   tmp = TS_LEFT, TS_LEFT = TS_BOT, TS_BOT = TS_RT, TS_RT = TS_TOP, TS_TOP = tmp;  break;
        case 0:   SWAP(TS_LEFT, TS_RT);  SWAP(TS_TOP, TS_BOT); break;
...
void loop()
{
    ...
    if (tp.z > MINPRESSURE && tp.z < MAXPRESSURE) {
        ...
        xpos = map(tp.x, TS_LEFT, TS_RT, 240, 0);
        ypos = map(tp.y, TS_TOP, TS_BOT, 400, 0);
